 ...
 */
const shiftUp = (heap: MinHeap, node: number, index: number) => {
...
// 新增，往 minHeap 新增节点
const push = (heap: MinHeap, node: number) => {
 ...
 * 获取第一个元素
 */
const peek = (heap: MinHeap) => heap[0] || null
 ...
 */
const shiftDown = (heap: MinHeap, node: number, i: number) => {
    let index = i
    ...
    while (index < halfLeangth) {
        ...
        if (compare(left, node) < 0) {
            // 首先右节点存在(右节点可能超过 length)，右节点比左节点小
            // right < left < node， right 最小
            ...
            // right > node, left > node，不用下沉了，直接 return 退出函数
            return
        }
    }
}
...
const pop = (heap: MinHeap) => {
    ...
    // pop 方法就直接 弹最后一个，中间不变。这种情况列举下：是 整个堆 的数据 都相等，取谁都一样那就取最后一个，既方便（省去下沉操作），又不改变堆的特性。
    if (first !== last) {
        // 这一步，将 last 节点放在首位
        heap[0] = last;
        // 执行下沉操作
